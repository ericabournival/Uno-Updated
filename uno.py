...
def main_loop(p1: list[tuple[int, str]], p2: list[tuple[int, str]], face_up: list[tuple[int, str]], deck: list[tuple[int, str]]) -> None:
    ...
    while len(p1) > 0 and len(p2) > 0:
        ...
        ans = int(input("Type '1' to play a card and type '2' to pick up a card.")) #Asks the user what they'll be doing turing their turn.
            
        if ans == 1: #If they choose to play a card.
            index = int(input("What is the index of the card you want to play? (starting at 1)")) - 1
        ...
        if ans == 2: #If they choose to pick up a card.
            p1.append(deck.pop(0)) 

        if len(deck) == 0: #In case the entirety of the deck has been used up, this should act as a failsafe.
            deck = [face_up.pop(0) for _ in range(len(face_up))] #Assigns the empty 'deck' to the cards in face_up and removes all of the face_up cards
            random.shuffle(deck) #Shuffles deck
            print("Deck has been reshuffled")
            face_up = [deck.pop(0)] #Takes a new face_up card.
        ...
        elif len(p1)== 0: #The opposite is also true, current turns into 1 as player 1 places down their last card.
